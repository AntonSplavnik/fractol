...
int handle_key(int key, void *param)
{
    ...
    else if (key == 6) // 'z' key to zoom in
    {
        ...
        p->offsetX = (p->offsetX - centerX) * ZOOM_SPEED + centerX;
        // p->offsetY = (p->offsetY - centerY) * ZOOM_SPEED + centerY;
        // p->offsetX = (p->offsetX - WIDTH / 2.0) * ZOOM_SPEED + WIDTH / 2.0;
        //p->offsetX = 0;
        printf("offsetX: %f\n", p->offsetX);
        // p->offsetY = (p->offsetY - HEIGHT / 2.0) * ZOOM_SPEED + HEIGHT / 2.0;
        printf("offsetY: %f\n", p->offsetY);
    }
...
int handle_mouse(int button, int x, int y, void *param)
{
    ...
    {
        p->zoom *= zoom_factor;
        p->offsetX = (1 - zoom_factor) * x + p->offsetX * zoom_factor;
        // p->offsetY = (1 - zoom_factor) * y + p->offsetY * zoom_factor;
    }
...
int main()
{
	...
	//draw_mandelbrot(mlx, win, zoom, offsetX, offsetY);
	//mlx_string_put(mlx, win, WIDTH * 0.45, HEIGHT * 0.9, 0xFF0000, "LazyPixel");
...
// 	//draw_circle(mlx, win, WIDTH * 0.5, HEIGHT * 0.5, size);
